...
class SolicitacaoService {
  ...
  static async update(req) {
    ...
    try {
      await Promise.all((await obj.itensSolicitacao).map(item => item.destroy({ transaction: t })));
      await Promise.all(
        itensSolicitacao.map(item =>
          obj.createItemSolicitacao(
            { quantidade: item.quantidade, solicitacaoId: obj.id, medicamentoId: item.medicamento.id },
            { transaction: t }
          )
        )
      );
  ...
  static async update(req) {
    ...
    try {
      await Promise.all((await obj.itensSolicitacao).map(item => item.destroy({ transaction: t }))); // destruindo todos itensSolicitacao desta solicitação
      await Promise.all(itensSolicitacao.map(item => obj.createItemSolicitacao({ quantidade: item.quantidade, tipoSanguineoId: item.tipoSanguineo.id, solicitacaoId: obj.id }, { transaction: t })));
